...
export default function AmbassadorPanel() {
  ...
  const statCards = [
    { label: "Total Referred", value: stats?.totalReferred || 0, icon: Users, color: "bg-blue-50 text-blue-600" },
    { label: "Pending", value: stats?.pendingProviders || 0, icon: Clock, color: "bg-yellow-50 text-yellow-600" },
    { label: "Approved", value: stats?.approvedProviders || 0, icon: CheckCircle, color: "bg-green-50 text-green-600" },
  ...
  const statusColor = (s: string) => {
    switch (s) {
      ...
      case "panding": return "bg-yellow-100 text-yellow-700";
  ...
  return (
    <div className="min-h-screen">
        ...
        {ambassador && (
          <div className="mt-4 bg-white border rounded-xl p-6 flex flex-wrap gap-6 items-center">
            <div>
              ...
              <p className="font-mono font-semibold">{ambassador.ambassadorId}</p>
            </div>
            <div>
              <p className="text-sm text-slate-500">Referral Code</p>
              <p className="font-mono font-semibold text-[#3ee0cf]">{ambassador.referralCode}</p>
            </div>
            <div>
              <p className="text-sm text-slate-500">City</p>
              <p className="font-semibold">{ambassador.assignedCity}, {ambassador.assignedCountry}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  jetsetcares.org/join/{encodeURIComponent(ambassador.referralCode)}
                </a>
                <button
                  onClick={() => {
      ...
      <div className="px-8 py-4">
        <h3 className="text-lg font-bold text-slate-800 mb-4">My Referred Providers</h3>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50">
                ...
                <TableHead className="font-bold text-center">Signup Date</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {providers.map((p) => (
                <TableRow key={p._id}>
                  ...
                  <TableCell className="text-center">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColor(p.userStatus || "")}`}>
              ...
              {providers.length === 0 && (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-10 text-slate-500">
                    No providers referred yet. Share your referral link to get started!
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
